...
import Materials from '@arasaac/api/models/Materials'
...
module.exports = {
  getMaterialById: (req, res) => {
    ...
    Materials.findOne({ idMaterial: id, status: PUBLISHED })
      ...
      .exec(async (err, material) => {
        ...
        if (!material) {
          return res.status(404).json({
            message: 'Material not found or not published yet',
            err,
          })
        }
  ...
  searchMaterials: (req, res) => {
    const { language: locale, searchText } = req.params
    ...
    Materials.find(
      { $text: { $search: searchText, $language: customLanguage }, status: PUBLISHED },
  ...
  getNewMaterials: (req, res) => {
    ...
    Materials.find({ lastUpdated: { $gt: startDate } })
      .sort({ lastUpdated: -1 })
      .populate('authors.author', 'name email company url facebook google')
      .lean()
      .exec(async (err, materials) => {
...
const getFiles = (material) => {
  ...
  return new Promise((resolve) => {
    ...
    let baseDir = path.resolve(config.MATERIAL_DIR, material.idMaterial.toString())
